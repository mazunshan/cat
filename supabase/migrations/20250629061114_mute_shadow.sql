...
-- 售后服务记录策略
CREATE POLICY "Users can read after sales records" ON after_sales_records
  FOR SELECT TO authenticated
  USING (true);

CREATE POLICY "After sales and admin can insert records" ON after_sales_records
  FOR INSERT TO authenticated
  WITH CHECK (
    auth.uid() IN (
...
CREATE POLICY "After sales and admin can update records" ON after_sales_records
  FOR UPDATE TO authenticated
  USING (
    auth.uid() IN (
      SELECT id FROM users WHERE role IN ('after_sales', 'admin')
...
CREATE POLICY "Users can insert their own attendance" ON attendance_records
  FOR INSERT TO authenticated
